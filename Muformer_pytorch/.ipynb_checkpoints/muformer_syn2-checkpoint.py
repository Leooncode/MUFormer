...
class Muformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.res_block1 = nn.Sequential(
            ...
            nn.BatchNorm2d(24, momentum=0.9),
            nn.LeakyReLU()
        )
        
        self.res_block2 = nn.Sequential(
            nn.Conv2d(24, 24, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3)),
            nn.BatchNorm2d(24, momentum=0.9),
            nn.LeakyReLU()
        ...
        self.conv2d = nn.Sequential(
            nn.Conv2d(24, 24, kernel_size=(1, 1), stride=(1, 1), padding=0),
        )
        ...
        self.cbam = CBAM(gate_channels=24, reduction_ratio=4, pool_types=['avg', 'max'])
    ...
    def change_enhancement_module(self, HSI_emb):
        ...
        HSI6_conv = self.res_block1(HSI[:, 6])
        HSI7_conv = self.res_block1(HSI[:, 7])
        HSI8_conv = self.res_block1(HSI[:, 8])
        HSI9_conv = self.res_block1(HSI[:, 9])
        HSI10_conv = self.res_block1(HSI[:, 10])
        HSI11_conv = self.res_block1(HSI[:, 11])
        HSI12_conv = self.res_block1(HSI[:, 12])
        HSI13_conv = self.res_block1(HSI[:, 13])
        HSI14_conv = self.res_block1(HSI[:, 14])
        ...
        HSI6_conv_1 = self.res_block2(HSI[:, 6])
        HSI7_conv_1 = self.res_block2(HSI[:, 7])
        HSI8_conv_1 = self.res_block2(HSI[:, 8])
        HSI9_conv_1 = self.res_block2(HSI[:, 9])
        HSI10_conv_1 = self.res_block2(HSI[:, 10])
        HSI11_conv_1 = self.res_block2(HSI[:, 11])
        HSI12_conv_1 = self.res_block2(HSI[:, 12])
        HSI13_conv_1 = self.res_block2(HSI[:, 13])
        HSI14_conv_1 = self.res_block2(HSI[:, 14])
        ...
        M6 = HSI6_conv - HSI[0, 5].unsqueeze(0)
        M7 = HSI7_conv - HSI[0, 6].unsqueeze(0)
        M8 = HSI8_conv - HSI[0, 7].unsqueeze(0)
        M9 = HSI9_conv - HSI[0, 8].unsqueeze(0)
        M10 = HSI10_conv - HSI[0, 9].unsqueeze(0)
        M11 = HSI11_conv - HSI[0, 10].unsqueeze(0)
        M12 = HSI12_conv - HSI[0, 11].unsqueeze(0)
        M13 = HSI13_conv - HSI[0, 12].unsqueeze(0)
        M14 = HSI14_conv - HSI[0, 13].unsqueeze(0)
        M15 = torch.zeros_like(HSI14_conv)
        ...
        M6_1 = HSI6_conv_1 - HSI[0, 5].unsqueeze(0)
        M7_1 = HSI7_conv_1 - HSI[0, 6].unsqueeze(0)
        M8_1 = HSI8_conv_1 - HSI[0, 7].unsqueeze(0)
        M9_1 = HSI9_conv_1 - HSI[0, 8].unsqueeze(0)
        M10_1 = HSI10_conv_1 - HSI[0, 9].unsqueeze(0)
        M11_1 = HSI11_conv_1 - HSI[0, 10].unsqueeze(0)
        M12_1 = HSI12_conv_1 - HSI[0, 11].unsqueeze(0)
        M13_1 = HSI13_conv_1 - HSI[0, 12].unsqueeze(0)
        M14_1 = HSI14_conv_1 - HSI[0, 13].unsqueeze(0)
        M15_1 = torch.zeros_like(HSI14_conv_1)

        M = torch.cat([M1, M2, M3, M4, M5, M6,
                       M7, M8, M9, M10, M11, M12,
                       M13, M14, M15], dim=0)  #(6, 18, 50, 50)
        M_t = torch.cat([M1_1, M2_1, M3_1, M4_1, M5_1, M6_1,
                         M7_1, M8_1, M9_1, M10_1, M11_1, M12_1,
                         M13_1, M14_1, M15_1], dim=0)
    ...
    def forward(self, Mu_HSI, mask = None):
        ...
        hp, wp = (h // p), (w // p)
        n = hp * wp
        HSI_test = Mu_HSI.contiguous().view(self.num_frames, self.channels, self.image_height, self.image_width)
        HSI_emb = self.encoder(HSI_test)
        HSI, A1, A2 = self.change_enhancement_module(HSI_emb)
        

        # M_mean = torch.mean(M, dim=(-2, -1))
        # M_mean = M_mean.unsqueeze(2).unsqueeze(3)
        # # M_mean = torch.abs(M_mean)
        
        
        
        # M_s_t = torch.mean(M_t, dim=(-2, -1))
        # M_s_t = M_s_t.unsqueeze(2).unsqueeze(3)
        ...
        x = x.squeeze(0).view(376, 24, 10, 10)
        x = self.cbam(x)
        x = x.unsqueeze(0).view(1, 376, 2400)
        
        cls_token = x[:, 0]
        cls_token = cls_token + 5 * (A1 * A2) * cls_token
        out = self.to_latent(cls_token)   # torch.Size([1, 1800])
        out = out.contiguous().view(self.num_frames, self.P, -1)  # torch.Size([6, 3, 100])
        abu = self.upscale(out).clamp_(0,1).contiguous().view(self.num_frames, self.P, self.image_height, self.image_width)

        abu = self.smooth(abu)
        abu_t = abu.reshape(15, self.P, self.image_height*self.image_width)
        ...
        out7 = self.decoder7(torch.transpose(abu_t[6], 1, 0))
        out8 = self.decoder8(torch.transpose(abu_t[7], 1, 0))
        out9 = self.decoder9(torch.transpose(abu_t[8], 1, 0))
        out10 = self.decoder10(torch.transpose(abu_t[9], 1, 0))
        out11 = self.decoder11(torch.transpose(abu_t[10], 1, 0))
        out12 = self.decoder12(torch.transpose(abu_t[11], 1, 0))
        out13 = self.decoder13(torch.transpose(abu_t[12], 1, 0))
        out14 = self.decoder14(torch.transpose(abu_t[13], 1, 0))
        out15 = self.decoder15(torch.transpose(abu_t[14], 1, 0))


        
        re_out = torch.stack([out1, out2, out3, out4, out5, out6, out7, out8, out9, out10, out11, out12, out13, out14, out15])
        re_out = re_out.permute(0, 2, 1).reshape(15, self.channels, self.image_height, self.image_width)
        return abu, re_out
